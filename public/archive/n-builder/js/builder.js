...
editableItems['.editBG, .white-bg, .secondary-bg'] = ['background-image', 'background-color'];
editableItems['nav a, a.edit'] = ['color', 'font-weight', 'text-transform'];
editableItems['.nav a, a'] = ['color', 'font-weight', 'text-transform'];
editableItems['h1, h2, h3, h4, h5, h6'] = ['color', 'font-size', 'background-color', 'font-family'];
editableItems['p, blockquote'] = ['color', 'font-size', 'background-color', 'font-family'];
...
var editableContent = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'ul', 'blockquote', '.editContent', '.navbar a', 'button', 'a.btn', '.footer a:not(.fa)', '.tableWrapper'];
	...
	el.sortable({
		...
		beforeStop: function(event, ui){
			...
			if( ui.item.find('.frameCover').size() == 0 ) {
				...
				bgButton = $('<button type="button" class="btn btn-warning bgBlock"><i class="fa fa-adjust"></i> secondary color</button>');
...
function styleClick(el) {
	...
	$('button#saveStyling').unbind('click').bind('click', function(){
		...
		if( $(el).hasClass('icon') ) {
			...
			//if the icons is being changed, save the old one so we can reset it if needed
			/*
			if( svgPath != $('select#icons').val() ) {
			
				$(el).uniqueId();
			
				_oldIcon[$(el).attr('data-svg-default')] = svgPath;
...
$(function(){
	...
	//secondary color
	$('#pageList').on("click", ".frameCover > .bgBlock", function(){
		var currentFrameWhite = $(this).closest('li').find('iframe').contents().find('.white-bg');
		var currentFrameOffWhite = $(this).closest('li').find('iframe').contents().find('.secondary-bg');

		$(currentFrameWhite).removeClass('white-bg').addClass('secondary-bg');
		$(currentFrameOffWhite).removeClass('secondary-bg').addClass('white-bg');
	});
	...
	*/
	...
	$('#styleSwitch').off('click', '#styleSwitchConfirm').on('click', '#styleSwitchConfirm', function(){
		...
		typographyCSSRef = $('#switchTypography').val().toLowerCase();
	...
	/*
	|----------------------------------------------------------------------------
	| START EXPORTER IMPORTER CODE
	|----------------------------------------------------------------------------
	*/
	$('#saveProjectPage').on('click', function(e){

		$('#savePage').click();

		if (navigator.appVersion.toString().indexOf('.NET') > 0){
			 saveTextAsFile('project.nemobuilt',JSON.stringify(localStorage));
		}else{
			console.save(JSON.stringify(localStorage),'project.nemobuilt');	
		}
 		
 	});

	$('#loadProjectPage').on('click', function(e){

		var file = document.getElementById("projectImport").files[0]; 
...
function saveTextAsFile(fileNameToSaveAs, textToWrite) {
    var ie = navigator.userAgent.match(/MSIE\s([\d.]+)/),
        ...
        ieVertion = (ie ? ie[1] : (ie11 ? 11 : -1));

    if (ie && ieVertion < 10) {
        console.log("No blobs on IE vertion < 10");
        return;
    }
